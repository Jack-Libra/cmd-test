...
class PacketProcessor:
    ...
    def __init__(self, registry: DefinitionRegistry):
        ...
        self.handlers = {
            ...
            "5F08": self._handle_5f08,
        }
        self.logger.info("封包處理器初始化")
    ...
    def _handle_5f08(self, packet: Dict):
        """處理5F08封包（號誌控制器現場操作）"""
        tc_id = packet.get("tc_id", 0)
        operation = packet.get("operation", 0)
        operation_details = packet.get("operation_details", {})
        
        self.logger.info(
            f"TC{tc_id:03d} 5F08: 號誌控制器現場操作 - {operation}, {operation_details}"
        )
